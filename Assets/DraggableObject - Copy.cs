...
public class DraggableObjectCopy : MonoBehaviour
{
    ...
    bool IsMoveValid(Vector3 tilePosition, Vector3 direction)
    {
        ...
        for (int x = 0; x < objectWidth; x++)
        {
            for (int y = 0; y < objectHeight; y++)
            {
                ...
                {
                    // Get the player's grid cell.
                    Vector3Int playerCell = restrictedTilemap.WorldToCell(player.transform.position);
                    // If the object is trying to move into a cell occupied by a player, prevent the move.
